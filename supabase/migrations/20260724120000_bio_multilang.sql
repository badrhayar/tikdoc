...
-- NOTE: `create or replace view` cannot reorder/rename existing columns, so the
-- new columns MUST be appended at the very END (after slot_minutes), keeping the
-- previous column list byte-for-byte identical. Order doesn't matter to the app
-- (mapDoctor reads columns by name).
